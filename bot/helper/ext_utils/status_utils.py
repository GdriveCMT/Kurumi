...
def get_readable_message(sid, is_user, page_no=1, status="All", page_step=1):
    ...
    msg += f"<b>CPU:</b> <code>{cpu_percent()}%</code> | <b>FREE:</b> <code>{get_readable_file_size(disk_usage(config_dict['DOWNLOAD_DIR']).free)}</code>"
    msg += f"\n<b>RAM:</b> <code>{virtual_memory().percent}%</code> | <b>UPTIME:</b> </b> <code>{get_readable_time(time() - botStartTime)}</code>"
    return msg, button
